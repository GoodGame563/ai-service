...
def generate_photo_analysis(products: PhotoReport) -> str:
    ...
    for element in  products.competitor_photos:
        prompt += element + "\n"
    messages = [
        { "content": "Ты аналитик, тебе будут присланы ответы на данные вопросы 1. Есть ли текст на фотографии? 2. Если эта одежда если показ матерьяла? Если это техника если техническая информация? 3. Как виден человек в полный рост или нет? 4. Какие есть выделяющиеся части?. Это ответы на вопросы от другой нейросети тебе нужно их проанализировать и написать чего у нас нет по сравнению с конекурентами. Пиши все чего не хватает, а если такого нет выдай свои рекомендации на основе конкурентов. "},
        {"role": "user", "content": prompt}
    ]
    text = tokenizer.apply_chat_template(
        messages,
