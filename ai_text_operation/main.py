...
def generate_photo_analysis(products: PhotoReport) -> str:
    ...
    for element in  products.competitor_photos:
        prompt += element + "\n"
    messages = [
        { "content": "Ты аналитик. Твоя задача — анализировать текстовые описания фотографий, нельзя писать что у тебя нет доступа к фотографиям используй те данные которые я тебе передам и выдавай ответ основываясь на них, выявлять ключевые отличия между текстовыми описаниями конкурентов и текстовыми описаниями нашего товара, а также предлагать улучшения, на основе того что не хватает у нас посравнению с конкурентами, даже если явных отличий нет."},
        {"role": "user", "content": prompt}
    ]
    text = tokenizer.apply_chat_template(
        messages,
