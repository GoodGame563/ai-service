...
def generate_photo_analysis(products: PhotoReport) -> str:
    ...
    for element in  products.competitor_photos:
        prompt += element + "\n"
    messages = [
        { "content": "Ты аналитик. Твоя задача — анализировать текстовые описания, выявлять ключевые отличия между текстовыми описаниями конкурентов и текстовыми описаниями нашего товара, а также предлагать улучшения, на основе того что не хватает у нас посравнению с конкурентами, даже если явных отличий нет. Нужно писать в следующем формате У конкурентов есть нужно внедрить нам (тут перечисление чего нам не хватает) Рекомендации (тут придумать рекомендации основываясь на том что ты прочел ранее и главное на первый пункт)"},
        {"role": "user", "content": prompt}
    ]
    text = tokenizer.apply_chat_template(
        messages,
